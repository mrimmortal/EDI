...
//      echo"</br></br>";
   ...
   //    print_r($dates);
	...
	public function trend_insident_data($start,$end)
	{
		 ...
		 $query = $this->db->query("SELECT status,
			 	...
			 	FROM `incident_list` WHERE logged_time >= '{$start}' AND updated_time <= '{$end}';");
	...
	public function trend_sr_data($start,$end)
	{
		 $query = $this->db->query("SELECT status,
	    	AVG(mttr) AS Avg_mmtr,
	    	COUNT(case status when 'Pending' then 1 else null end) AS Pending,
			...
			COUNT(case when priority ='S4' AND (status='Pending' OR status='In-Progress') then 1 else null end) AS Others,		
	    	COUNT(sr_id) As sr_count 
	    	FROM `sr_list` WHERE log_time >= '{$start}' AND updated_time <= '{$end}';");
